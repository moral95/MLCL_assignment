...
def select_criterion(loss_function):
    if loss_function == 'BCE':
        ...
        criterion = nn.MSELoss() # Least Squares Loss(Means Square Error)
    else:
...
def training(config):
    ...
    netD = Discriminator(config['training']['ngpu'],
                        config['model']['nc'],
                        config['model']['ndf']
                         ).to(device)
    ...
    criterion = select_criterion(config['training']['loss_function']).to(device)
    ...
    optimizerD = torch.optim.Adam(netG.parameters(), lr= config['training']['lr'])
    optimizerG = torch.optim.Adam(netD.parameters(), lr= config['training']['lr'])
    ...
    for epoch in range(config['training']['num_epochs']):
        for i, data in enumerate(train_loader, 0):
            ...
            #     nn.utils.clip_grad(, 1.0) # Clip the gradient function
